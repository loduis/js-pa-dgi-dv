...
class DV {
  ...
  get rucTable () {
    ...
    if (this.len === 4) {
      if (this.ruc[1] !== 'NT') return null
      const [province, , imageFolio, seatToken] = this.ruc
      if (
        isNaN(province) ||
        isNaN(imageFolio) ||
        isNaN(seatToken) ||
    ...
    let [letter, imageFolio, seatToken] = this.ruc
    let ructb = N
